...
router.get("/getgoodlist",async(req,res)=>{
    const {cateid,page,size,type} = req['query'];
    if(!cateid || !page || !size){
        res.send(MError("缺少必要条件"));
        return;
    }
    ...
    let sql = `SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} ${where}`;
    // console.log(sql)
    if(size && page){
    ...
    // 组装商品数据和总页码
    res.send(Success({totalPage,goodData}));
});
...
router.get('/sms',async(req,res)=>{
    ...
    if(nowtime - addtime > 300000){//如果超过5分钟就重新获取  注意此处不再向手机发送短信验证码
...
router.post('/wxlogin',async (req,res)=>{
   ...
   var APPID = 'wx9f401fb7e15004e0';                //换成自己的appid公众后台获取
   var SECRET = '4aecbf9299d55768e7b489f68850fb04'; //换成自己的秘钥去公众后台获取
...
//	商城消息推送接口 每周1的1点1分30秒触发
router.get('/push',async(req,res)=>{
	
	// 获取最新一条推送消息
	let data = await Db.select(req, `SELECT * FROM message  order by addtime desc  LIMIT 1` );
	let info = data[0].desc;
	//开始定时任务
	scheduleCronstyle(info);
	// console.log(info);
	res.send(Success([],'监听成功'));
